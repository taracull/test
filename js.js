...
function loadData() {
  fetch('database.json')
    ...
    .then(data => {
      ...
      // Set default value for carModel dropdown
      carModelDropdown.value = 'Car 1';
      ...
      // Function to populate the accessory dropdown
      function populateAccessoryDropdown(selectedCarModel) {
        const accessories = [...new Set(data.filter(item => item['Car Model'] === selectedCarModel).map(item =>
          item['Accessory']))];
      ...
      // Call the function to populate the accessory dropdown initially
      populateAccessoryDropdown(carModelDropdown.value);

      // Set default value for accessory dropdown
      accessoryDropdown.value = 'All';

      // Event listener for carModel dropdown change event
      carModelDropdown.addEventListener('change', () => {
        const selectedCarModel = carModelDropdown.value;
        populateAccessoryDropdown(selectedCarModel);
      });
